...
if (empty($session_id)) {
    ...
    if (isset($_GET['kakutei_btn'])) {
        ...
        if (isset($_GET['one_now_sql_zensuu']) && $_GET['one_now_sql_zensuu'] != "") {
            ...
            $get_now_sql = $_SESSION['four_five_default_SQL'];
            $_SESSION['back_five_default_root_sql_zensuukanryou'] = $get_now_sql;
            dprint("ここ:back_five_default_root_sql_zensuukanryou");
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // キャンセル登録 処理
    if (isset($_GET['update_to_cancel'])) {
        ...
        $sql = "UPDATE HTPK SET ピッキング数量 = 0, 処理Ｆ = 8, 
        処理終了日時 = TO_DATE(:end_time, 'YYYY-MM-DD HH24:MI:SS')  
        WHERE 処理ＳＥＱ = :Syori_SEQ";
        ...
        oci_bind_by_name($stid, ":end_time", $end_time); // 処理終了日時
        oci_bind_by_name($stid, ":Syori_SEQ", $five_back_Syori_SEQ); // 処理 SEQ
    ...
    if (isset($_GET['all_canceled_button'])) {
        // GET パラメータをセッションに格納
        $_SESSION['all_canceled_params'] = $_GET;
        ...
        <script>
            document.getElementById("confirmYesButton").addEventListener("click", function() {
                ...
                $.ajax({
                    ...
                    type: "GET",
                    data: {
                        five_back_Syori_SEQ: "' . $_SESSION['all_canceled_params']['five_back_Syori_SEQ'] . '",
                        update_to_cancel: true
                    },
                    success: function(response) {
                        window.location.href = "./four.php?" + $.param(' . json_encode($_SESSION['all_canceled_params']) . ');
                    }
                });
            });
...
<body>
    ...
    <div class="head_box_02">
        <div class="head_content_02">
            ...
            <span class="all_cancel">
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="GET" name="all_canceled_post" id="all_canceled_post">
                    <!-- 2024/07/04 変更 -->
                    ...
                    <button type="submit" name="all_canceled_button" id="all_canceled_button">出荷キャンセル</button>
                </form>
            </span>
        </div>
    </div> <!-- ===============  head_box_02 END =============== -->
